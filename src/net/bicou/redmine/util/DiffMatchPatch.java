...
public class DiffMatchPatch {
	...
	public void diff_cleanupSemantic(LinkedList<Diff> diffs) {
		...
		while (thisDiff != null) {
			if (thisDiff.operation == Operation.EQUAL) {
				...
				// sides of it.
				if (lastequality != null && (lastequality.length() <= Math.max(length_insertions1, length_deletions1)) && (lastequality.length() <= Math.max
	...
	public void diff_cleanupEfficiency(LinkedList<Diff> diffs) {
         ...
         */
				if (lastequality != null && ((pre_ins && pre_del && post_ins && post_del) || ((lastequality.length() < Diff_EditCost / 2) && ((pre_ins ? 1 :
			...
			while (!bigpatch.diffs.isEmpty()) {
				...
				while (!bigpatch.diffs.isEmpty() && patch.length1 < patch_size - Patch_Margin) {
					...
					if (diff_type == Operation.INSERT) {
						...
						empty = false;
					} else if (diff_type == Operation.DELETE && patch.diffs.size() == 1 && patch.diffs.getFirst().operation == Operation.EQUAL && diff_text
							.length() > 2 * patch_size) {
	...
	private static String unescapeForEncodeUriCompatability(String str) {
		return str.replace("%21", "!").replace("%7E", "~").replace("%27", "'").replace("%28", "(").replace("%29", ")").replace("%3B", ";").replace("%2F",
				"/").replace("%3F", "?").replace("%3A", ":").replace("%40", "@").replace("%26", "&").replace("%3D", "=").replace("%2B", "+").replace("%24",
