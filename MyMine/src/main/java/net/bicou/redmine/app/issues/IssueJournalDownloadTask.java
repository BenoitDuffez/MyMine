 ...
 * thread.
 */
public class IssueJournalDownloadTask extends IssueHistoryDownloadTask<IssueJournal> {
	...
	public interface JournalsDownloadCallbacks {
		 ...
		 */
		void onJournalsDownloaded(IssueJournal history);
	...
	public IssueJournalDownloadTask(final ActionBarActivity act, final JournalsDownloadCallbacks callbacks, final Issue issue) {
		super(act, issue);
	...
	@Override
	protected IssueJournal downloadAndParse(DbAdapter db) {
		final IssueJournal history = downloadHistory();
		if (history == null || history.journals == null) {
			return null;
		}
		parseJournals(history.journals, db);
		return history;
	}
	 ...
	 */
	private IssueJournal downloadHistory() {
		// Download issue JSON
