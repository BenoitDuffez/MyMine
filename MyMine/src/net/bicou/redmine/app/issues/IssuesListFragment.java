...
public class IssuesListFragment extends SherlockListFragment implements LoaderCallbacks<Cursor>, SplitScreenFragmentConfigurationChangesListener {
	...
	public void onListItemClick(final ListView l, final View v, final int position, final long id) {
		...
		if (mIsSplitScreen) {
			getFragmentManager().beginTransaction().replace(R.id.issues_pane_issue, frag).commit();
		} else {
			getSherlockActivity().getSupportActionBar().setNavigationMode(ActionBar.NAVIGATION_MODE_STANDARD);
			getFragmentManager().beginTransaction().replace(R.id.issues_pane_list, frag).addToBackStack("prout").commit();
		}
	}
