	...
	public static class DbManager extends SQLiteOpenHelper {
		...
		public void onUpgrade(final SQLiteDatabase db, final int oldVersion, final int newVersion) {
			...
			if (oldVersion < 4) {
				...
				try {
					db.execSQL("UPDATE " + ProjectsDbAdapter.TABLE_PROJECTS + " SET " + ProjectsDbAdapter.KEY_SERVER_ID + " = (SELECT MIN(" + KEY_ROWID + ")" +
							" " +
			...
			if (oldVersion < 5) {
				...
				try {
					...
					sql += " SELECT " + Util.joinFirstWords(ProjectsDbAdapter.PROJECT_FIELDS, ", ").replace(KEY_ROWID + ",", "id,");
					sql += " FROM tmp";
					db.execSQL(sql);
